...
function removeFirstOccurrences(str, value) {
  ...
  const newStr = str.substring(0, str.indexOf(value));
  const newStr2 = str.substring(str.indexOf(value) + value.length);
...
function removeLastOccurrences(str, value) {
  ...
  const newStr2 = str.slice(str.lastIndexOf(value) + value.length);
  return newStr.concat(newStr2);
}
...
function isPalindrome(str) {
  const newStrb = str.toLowerCase().replace(/[^a-z]/gi, '');
  let revNum = '';
  for (let i = newStrb.length - 1; i >= 0; i -= 1) {
    revNum += newStrb[i];
  }

  return newStrb === revNum;
}
...
function reverseWords(str) {
  const newStrb = str.split(' ');
  const arrayNew = [];
  newStrb.forEach((element) => {
    let revNum = '';
    for (let i = element.length - 1; i >= 0; i -= 1) {
      revNum += element[i];
    }
    arrayNew.push(revNum);
  });
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].toUpperCase() === str[i]) {
      revNum += str[i].toLowerCase();
    } else {
      revNum += str[i].toUpperCase();
    }
  }
  return revNum;
}
